...
class ProdutosState extends ChangeNotifier {
  final ProdutosServices _services;
  ProdutosState(this._services);

  List<ProdutosModel> data = [];
  ...
  Future<bool> insert(BuildContext context, InsertProdutosModel modelo) async {
  ...
  Future<InsertProdutosModel?> readById(BuildContext context, String id, String codigoBarra) async {
    loading = Loading.loadInsertPage;
    // notifyListeners();

    final res = await _services.readById(id, codigoBarra);
  ...
  Future<bool> insertBaixaEstoque(BuildContext context, String idProdutos, int estoque, String dataBaixa) async {
    final res = await _services.insertBaixaEstoque(idProdutos, estoque, dataBaixa);
  ...
  Future<({String nome, String preco})?> readByBarsCode(BuildContext context, String codigoBarra) async {
    final res = await _services.readByBarsCode(codigoBarra);

    if (!context.mounted) return null;

    if (res.$2?.type == TypeErrorModel.notAuthenticated) {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const LoginPage()));
      return null;
    }
    if (res.$2?.type == TypeErrorModel.notAllowed || res.$2?.type == TypeErrorModel.common) {
      showErrorMessage(context, message: res.$2!.message, success: false);
      return null;
    }
