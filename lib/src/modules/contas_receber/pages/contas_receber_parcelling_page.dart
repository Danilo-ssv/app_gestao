...
class ContasReceberParcellingPage extends StatefulWidget {
  final String id;
  final Function onSave;
  const ContasReceberParcellingPage({super.key, required this.id, required this.onSave});

  @override
  State<ContasReceberParcellingPage> createState() => _ContasReceberParcellingPageState();
}

class _ContasReceberParcellingPageState extends State<ContasReceberParcellingPage> {
  ...
  void insert() async {
    if (_parcelasController.text.isEmpty) {
      showErrorMessage(context, message: 'Parcela é Obrigatório!', success: false);
      return;
    }

    final res = await context.read<ContasReceberState>().parcelling(
          context,
          widget.id,
